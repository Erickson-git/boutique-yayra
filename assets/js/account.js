/* Badge de profil global : quand un compte est connecté, affiche l'avatar
   (photo de profil) dans un coin en haut, avec un menu déroulant contenant le
   lien vers l'espace, le changement de photo et le bouton « Se déconnecter ».
...
.ya-acc-menu{position:absolute; top:calc(100% + 10px); right:0; min-width:210px; background:var(--bg,#1c140b); color:var(--ink,#f3ead9);
...
`;
  ...
  function buildMenu(u){
    ...
    m.innerHTML =
      ...
      + '<button class="ya-acc-item danger" type="button" data-logout><svg viewBox="0 0 24 24"><path d="M15 12H4M9 7l-5 5 5 5"/><path d="M14 4h5v16h-5"/></svg> Se déconnecter</button>';
    return m;
  }
  ...
  function render(u){
    ...
    // Placement : dans la navbar si présente, sinon flottant en haut à droite
    ...
    menu.querySelector('[data-photo]').addEventListener('click', ()=> file.click());
    file.addEventListener('change', ()=>{
      if(file.files && file.files[0]) setAvatarFromFile(file.files[0], ()=>{
